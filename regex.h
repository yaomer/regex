...
class matcher {
public:
    matcher(const std::string& expr) : nfa(build(expr)) {  }
...
private:
    ...
    {
        auto postfix_expr = to_postfix(add_concat_operator(expr));
        return build_from_postfix(postfix_expr);
    }
    NFA build_from_postfix(const std::string& postfix_expr)
    {
        std::stack<NFA> stk;
